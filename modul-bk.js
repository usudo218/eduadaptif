...
<head>
    ...
    <title>Modul Berpikir Komputasional - EduAdaptif SMA</title>
    <style>
        :root {
            --warna-utama: #E11D48; /* Warna Rose/Crimson untuk BK */
            --warna-aksen: #059669; 
            --warna-merah: #EF4444;
            --warna-kuning: #F59E0B; 
            --warna-latar: #F3F4F6; 
        ...
        * { box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .header { background: var(--warna-utama); color: white; padding: 20px; text-align: center; position: relative;}
        .header h1 { margin: 0; font-size: 1.5rem; }
        .header p { margin: 5px 0 0; font-size: 0.9rem; opacity: 0.9; }
        
        .back-btn { position: absolute; top: 15px; left: 15px; color: white; text-decoration: none; font-size: 0.9rem; background: rgba(255,255,255,0.2); padding: 6px 10px; border-radius: 5px; transition: 0.2s;}
        .back-btn:hover { background: rgba(255,255,255,0.4); }
        .logout-btn { position: absolute; top: 15px; right: 15px; color: white; text-decoration: none; font-size: 0.9rem; background: rgba(220, 38, 38, 0.9); padding: 6px 12px; border-radius: 5px; transition: 0.2s; border: none; cursor: pointer; font-weight: bold;}
        ...
        .login-illustration { width: 80%; max-width: 220px; margin-bottom: 20px; display: block; margin-left: auto; margin-right: auto; border-radius: 12px;}
        ...
        select.input-nama:focus { border-color: var(--warna-utama); background-color: white; outline: none; box-shadow: 0 0 0 3px rgba(225, 29, 72, 0.2);}
        ...
        .teks-materi h4 { margin-top: 15px; color: var(--warna-utama); margin-bottom: 5px;}
        .ilustrasi-mini { width: 100%; height: auto; min-height: 150px; background-color: #e5e7eb; object-fit: cover; border-radius: 8px; margin: 15px 0; border: 1px dashed #9ca3af; }
        
        .tabel-materi { width: 100%; border-collapse: collapse; margin: 10px 0; font-size: 0.9rem;}
        .tabel-materi th, .tabel-materi td { border: 1px solid #D1D5DB; padding: 8px; text-align: left; }
        .tabel-materi th { background-color: var(--latar-mentor); color: var(--warna-utama);}
        ...
        .btn-action:hover { background: #BE123C; }
...
<body>
    ...
    <div class="app-container">
        <div class="header">
            <a href="index.html" class="back-btn" onclick="hapusProgres()">🔙 Dashboard</a>
            <button id="btn-logout" class="logout-btn" onclick="logoutSiswa()" style="display: none;">🚪 Logout</button>

            <h1>Modul Berpikir Komputasional</h1>
        ...
        <div class="content">
            <div id="area-login" class="area-konten area-aktif">
                <div class="login-card">
                    <img src="images/login-bk.png" alt="Ilustrasi Login BK" class="login-illustration" onerror="this.style.display='none'">
                    <h2>Selamat Datang, Problem Solver!</h2>
                    <p>Mari asah otakmu untuk memecahkan masalah layaknya seorang ilmuwan komputer. Pilih identitasmu.</p>
                    ...
                    <button class="btn-action btn-submit" onclick="simpanNamaSiswa()" style="margin-top: 10px;">Latih Otak Sekarang</button>
                </div>
            </div>

            <div id="area-mentor-global" style="display: none;">
                <div class="mentor-box" id="kotak-mentor">
                    <div class="mentor-avatar" id="avatar-mentor">👨‍🏫</div>
            ...
            <div id="area-materi" class="area-konten">
                <div class="teks-materi">
                    <h3>1. Empat Fondasi Berpikir Komputasional</h3>
                    <p><em>Computational Thinking</em> adalah cara berpikir untuk menyelesaikan masalah (problem solving). Ada 4 fondasi utama:</p>
                    <img src="images/ct.jpg" alt="Ilustrasi Computational Thinking" class="ilustrasi-mini" onerror="this.style.display='none'">
                    <table class="tabel-materi">
                        <tr><th>Fondasi</th><th>Penjelasan</th></tr>
                        <tr><td><strong>Dekomposisi</strong></td><td>Memecah masalah besar menjadi bagian-bagian kecil agar mudah diselesaikan.</td></tr>
                        <tr><td><strong>Pengenalan Pola</strong></td><td>Mencari kesamaan atau pola di antara masalah-masalah yang ada.</td></tr>
                        <tr><td><strong>Abstraksi</strong></td><td>Membuang informasi yang tidak penting dan fokus pada hal yang inti saja.</td></tr>
                        <tr><td><strong>Algoritma</strong></td><td>Menyusun langkah-langkah atau instruksi yang berurutan untuk menyelesaikan masalah.</td></tr>
                    </table>

                    <h3>2. Algoritma Pencarian (Searching)</h3>
                    <p>Mencari data spesifik di dalam sekumpulan data.</p>
                    <img src="images/searching.jpg" alt="Ilustrasi Searching" class="ilustrasi-mini" onerror="this.style.display='none'">
                    <ul>
                        <li><strong>Sequential/Linear Search:</strong> Mencari data satu per satu dari awal sampai akhir. Cocok untuk data acak.</li>
                        <li><strong>Binary Search:</strong> Menebak langsung ke nilai tengah, membuang setengah data yang tidak relevan. <strong>Syarat utamanya: Data harus SUDAH TERURUT.</strong></li>
                    </ul>

                    <h3>3. Algoritma Pengurutan (Sorting)</h3>
                    <p>Mengatur elemen-elemen ke dalam urutan tertentu (<strong>Ascending</strong> = Naik/Kecil ke Besar, <strong>Descending</strong> = Turun/Besar ke Kecil).</p>
                    <img src="images/sorting.jpg" alt="Ilustrasi Sorting" class="ilustrasi-mini" onerror="this.style.display='none'">
                    <p>Contoh algoritmanya: <em>Bubble Sort</em> (gelembung menukar posisi yang bersebelahan), <em>Insertion Sort</em>, dan <em>Selection Sort</em>.</p>

                    <h3>4. Struktur Data: Stack & Queue</h3>
                    <p>Bagaimana cara komputer menyimpan dan mengatur antrean data?</p>
                    <img src="images/stack-queue.jpg" alt="Ilustrasi Stack dan Queue" class="ilustrasi-mini" onerror="this.style.display='none'">
                    <ul>
                        <li><strong>Stack (Tumpukan):</strong> Prinsipnya <strong>LIFO</strong> (<em>Last In, First Out</em>). Yang terakhir masuk, pertama kali keluar. Contoh: Tombol Undo (CTRL+Z) di Word atau tumpukan piring.</li>
                        <li><strong>Queue (Antrean):</strong> Prinsipnya <strong>FIFO</strong> (<em>First In, First Out</em>). Yang pertama masuk, pertama dilayani. Contoh: Antrean cetak dokumen (Printer spooler) atau antrean kasir.</li>
                    </ul>
            ...
            <div id="area-kuis" class="area-konten question-area">
                ...
                <button class="btn-action btn-bantuan" id="tombol-bantuan" onclick="mintaBantuan()">💡 Perlu Bantuan Pak Agus?</button>
    ...
    <script>
        ...
        const URL_APPS_SCRIPT = "PASTE_URL_APPS_SCRIPT_ANDA_DI_SINI"; 
        ...
        function simpanProgres(faseSistem) {
            ...
            // Kunci LocalStorage KHUSUS MODUL BK
            localStorage.setItem('eduAdaptif_ModulBK', JSON.stringify(progresState));
        }

        function muatProgres() {
            const simpanan = localStorage.getItem('eduAdaptif_ModulBK');
            if(simpanan) {
                ...
                if (p.fase === 'materi') { tampilkanMateri(); } else if (p.fase === 'kuis') { ubahTampilan('area-kuis'); aturMentor("normal"); document.getElementById('pesan-mentor').innerText = `Halo, ${namaSiswa}! Analisamu belum selesai, ayo lanjutkan kuisnya.`; muatSoal(); }
            } else { inisialisasiDropdown(); }
        }

        function hapusProgres() { localStorage.removeItem('eduAdaptif_ModulBK'); }

        function logoutSiswa() {
            if(confirm("Yakin ingin ganti akun? Progres kuis saat ini akan diulang dari awal.")) {
                hapusProgres(); window.location.reload();
            }
        }
        ...
        function aturMentor(mode) {
            ...
            if(mode === "normal") { kotak.classList.remove('peringatan'); nama.classList.remove('peringatan'); document.getElementById('avatar-mentor').innerText = "👨‍🏫"; } 
            else if (mode === "bantuan") { kotak.classList.add('peringatan'); nama.classList.add('peringatan'); document.getElementById('avatar-mentor').innerText = "💡"; }
        }

        function tampilkanMateri() {
            ubahTampilan('area-materi'); aturMentor("normal");
            document.getElementById('pesan-mentor').innerText = `Halo, ${namaSiswa} dari ${kelasSiswa}! Mari asah logikamu dengan materi Computational Thinking ini.`;
            indeksSoalSekarang = 0; jumlahBenar = 0; totalKaliBantuan = 0; simpanProgres('materi'); 
        }

        function mulaiKuis() {
            ubahTampilan('area-kuis'); aturMentor("normal");
            document.getElementById('pesan-mentor').innerText = `Gunakan logikamu, ${namaSiswa}! Jawab 15 soal CT ini.`;
            jumlahPercobaan++; 
            if(typeof daftarSoalAsli === 'undefined') { alert("File soal-bk.js tidak terbaca!"); return; }
        ...
        function muatSoal() {
            ...
            const btnBantuan = document.getElementById('tombol-bantuan'); btnBantuan.style.display = "block"; btnBantuan.disabled = false; btnBantuan.classList.remove('btn-disabled'); btnBantuan.innerText = "💡 Perlu Bantuan Pak Agus?";
            aturMentor("normal"); document.getElementById('pesan-mentor').innerText = `Baca dengan teliti kasusnya.`;
        ...
        function mintaBantuan() {
            ...
            const btnBantuan = document.getElementById('tombol-bantuan'); btnBantuan.disabled = true; btnBantuan.classList.add('btn-disabled'); btnBantuan.innerText = "✔️ Bantuan Telah Diberikan";
            simpanProgres('kuis'); 
        }
        ...
        function tampilkanHasil() {
            ...
            let statusSiswa = ""; 
            if (skorAkhir >= KKM) {
                document.getElementById('pesan-mentor').innerText = `Luar Biasa, ${namaSiswa}! Logika pemecahan masalahmu (CT) tuntas 100% TANPA bantuan.`;
                ...
                document.getElementById('pesan-mentor').innerText = (skorAwal === 100 && totalKaliBantuan > 0) ? `Skor 100, tapi ada jejak bantuan. Pemikir hebat harus mandiri. Ulangi!` : `Skormu ${skorAkhir}. Masih ada algoritma pikiran yang salah. Ayo remedial!`;
            ...
            if(URL_APPS_SCRIPT !== "https://script.google.com/macros/s/AKfycbzPcanC73JEFcHGfgKWDRfaUTNaztpOGZSmM61VqV_BfkiEYWebWNSHnZTE7Zx-4XDn/exec") {
                const dataKirim = new URLSearchParams({ namaSiswa: namaSiswa + " (" + kelasSiswa + ")", modul: "Berpikir Komputasional (BK)", skor: skorAkhir, bantuan: totalKaliBantuan, percobaan: jumlahPercobaan, status: statusSiswa });
                fetch(URL_APPS_SCRIPT, { method: 'POST', body: dataKirim }).then(response => { console.log("Data BK terekam."); }).catch(error => { console.error("Gagal kirim:", error); });
            }
        }
